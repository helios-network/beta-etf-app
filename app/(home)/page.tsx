...
export default function Home() {
  ...
  const { data: etfsData } = useQuery({
    ...
    queryFn: () =>
      fetchETFs(1, 10, isReversed ? undefined : selectedDepositToken?.address),
    ...
    try {
      if (!isReversed) {
        ...
        const sharesReceived =
          Number(result.sharesOut) / Number(sharesMultiplier)

        // Format the assets purchased
        let assetsMessage = ""
        if (
          result.amountsOut &&
          result.amountsOut.length > 0 &&
          selectedETF.assets
        ) {
        ...
        const depositDecimals = selectedETF.depositDecimals || 18
        const depositMultiplier = BigInt(10) ** BigInt(depositDecimals)
        const [minOutInteger = "0", minOutFractional = ""] =
          buyAmount.split(".")
      ...
      <Card className={s.form}>
                ...
                >
                  {selectedDepositToken ? (
                    <span className={s.tokenButtonContent}>
                      {(() => {
                        const logo =
                          tokenData[selectedDepositToken.symbol.toLowerCase()]
                            ?.logo
                        return logo ? (
                          <Image
                            src={logo}
            ...
            <div className={s.field}>
              ...
              <div className={s.bottom}>
                {sellAmount && selectedETF?.sharePrice ? (
                  `~$${(
                    parseFloat(sellAmount) * parseFloat(selectedETF.sharePrice)
                  ).toFixed(2)}`
        ...
        {!isWalletConnected ? (
          ...
          <Button className={s.start} disabled>
            Wrong Network (
            {CHAIN_CONFIG[selectedETF.chain]?.name ??
              `Chain ${selectedETF.chain}`}
            )
          </Button>
